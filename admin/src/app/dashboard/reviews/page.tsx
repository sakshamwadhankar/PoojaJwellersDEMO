  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-neutral-400 mt-1 text-sm">Manage customer reviews. Toggle visibility without deleting.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: "Hidden", value: reviews.filter((r) => !r.is_visible).length, color: "text-red-400" },
        ].map((s) => (
        ...
        {(["all", "visible", "hidden"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-all ${
              filter === f ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" : "text-neutral-500 hover:text-neutral-300 border border-transparent"}`}>
            {f}
          </button>
        ))}
      </div>
      ...
      {loading
        ...
        : filtered.length === 0
          ? <div className="bg-white/5 border border-white/10 rounded-2xl p-10 text-center text-neutral-500">No reviews found.</div>
          : <div className="space-y-3">
              {filtered.map((r) => (
                <div key={r.id} className={`bg-white/5 border rounded-2xl p-5 transition-all ${r.is_visible ? "border-white/10" : "border-red-500/20 opacity-60"}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-amber-500/20 text-amber-400 text-sm font-bold flex items-center justify-center flex-shrink-0">
                          {r.customer_name?.charAt(0)?.toUpperCase() ?? "?"}
                        </div>
                        <div>
                          <p className="text-white font-medium text-sm">{r.customer_name}</p>
                          <StarRating rating={r.rating} />
                        </div>
                        {!r.is_visible && (
                          <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 font-medium">Hidden</span>
                        )}
                      </div>
                      <p className="text-neutral-400 text-sm leading-relaxed">{r.review_text}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {/* Toggle visibility */}
                      <button onClick={() => toggleVisibility(r)} title={r.is_visible ? "Hide review" : "Show review"}
                        className={`p-2 rounded-lg transition-all ${r.is_visible ? "text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20" : "text-neutral-500 bg-white/5 hover:bg-white/10"}`}>
                        {r.is_visible
