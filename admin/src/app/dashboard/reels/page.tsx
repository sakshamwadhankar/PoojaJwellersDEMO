  ...
  return (
    <div className="py-6 max-w-5xl">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">Manage video reels. Upload MP4s or paste direct video URLs.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Upload mode toggle */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Video Source</label>
              <div className="flex gap-2 bg-gray-50 border border-gray-200 rounded-xl p-1">
                {(["url", "file"] as const).map((m) => (
                  <button key={m} type="button" onClick={() => setUploadMode(m)}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${uploadMode === m ? "bg-white text-black shadow-sm border border-gray-200" : "text-gray-500 hover:text-gray-700 border border-transparent"}`}>
            ...
            {uploadMode === "url" ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Video URL (.mp4)</label>
            ...
            <div>
              ...
              <input type="number" min={1} value={displayOrder} onChange={(e) => setDisplayOrder(Number(e.target.value))}
                className="w-full px-4 py-2.5 rounded-xl bg-white border border-gray-300 text-gray-900 focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition text-sm" />
              <p className="text-gray-500 text-xs mt-1">Lower number = shows first</p>
            </div>
          ...
          {loading
            ...
            : reels.length === 0
              ...
              : <div className="space-y-3 max-h-[640px] overflow-y-auto pr-1">
                  {reels.map((reel) => (
                    <div key={reel.id} className="bg-white border border-gray-200 shadow-sm rounded-xl p-4 hover:border-gray-300 transition-colors group">
                      <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              <input type="number" min={1} defaultValue={reel.display_order}
                                ...
                                className="w-16 px-2 py-0.5 rounded-md bg-white border border-gray-300 text-gray-900 text-xs focus:outline-none focus:border-black" />
                            </div>
                        ...
                        <button onClick={() => handleDelete(reel)}
                          className="p-2 rounded-lg text-gray-400 hover:text-black hover:bg-gray-100 transition-all opacity-0 group-hover:opacity-100 flex-shrink-0 mt-1">
